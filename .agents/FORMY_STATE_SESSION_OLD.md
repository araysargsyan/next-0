...
**Status:** ✅ RESOLVED (July 16, 2026). Decision made and implemented — see commit `019a48f8`.

---

## Current TODO List (as of July 16, 2026)

### 🚨 Ultra Priority

- [x] **19. RSC claim vs. reality — architecture decision required:**
  Resolved. Decision made and implemented in commit `019a48f8`.
...
## 20. Architecture v13.0: FormyRestoreEngine & FormContent Architecture (July 16, 2026)

### Objective
Resolve the RSC claim vs. reality gap (§19) by converting `FormyInput` into a pure React Server Component (RSC) that has zero client JS and does not require wrappers or `cloneElement` rendering.

### Architectural Decisions

#### 1. Pure RSC FormyInput
Instead of passing inputs through a client component wrapper (`DynamicInput`), we render the `<input>` directly as a pure RSC, attaching a simple `data-formy-input` data attribute. This matches our architectural promise of 100% server-rendered inputs.

#### 2. Event Delegation & Dynamic Restore Engine
All user input captures and restoration logic are centralized inside a dynamically loaded `<FormyRestoreEngine />` component, rendered once per form:
- Restores inputs (`input`, `select`, `textarea`, `checkbox`, `radio`) based on saved values.
- Listens to native `input` and `change` events on the parent form via event delegation.

#### 3. Resolving the Null Form Ref Issue
During initial component mounting and hydration, forwarding refs from wrapper components (like `next/form`'s `<Form>`) can result in the `formRef.current` being `null` when children's layout effects execute. 
- **Solution:** Instead of passing the parent's `formRef` down as a prop, `FormyRestoreEngine` renders a hidden `<span ref={containerRef} style={{ display: "none" }} />` element, and obtains the parent form instance dynamically in its layout effects using `containerRef.current?.closest("form")`. This completely eliminates the timing/null-ref dependency.
